...
def nc2np(path, variables, years, save_dir, partition, resolution, num_shards_per_year):
    ...
    for year in tqdm(years):
        ...
        for var in variables:
            ...
            if len(ds[code].shape) == 3:  # surface level variables
                ...
                np_vars[var] = ds[code].to_numpy()[:HOURS_PER_YEAR:6]
                ...
                for level in all_levels:
                    ...
                    level = int(level)
                    # remove the last 24 hours if this year has 366 days
                    np_vars[f"{var}_{level}"] = ds_level[code].to_numpy()[:HOURS_PER_YEAR:6]
        ...
        num_hrs_per_shard = HOURS_PER_YEAR // (6 * num_shards_per_year)
        for shard_id in range(num_shards_per_year):
            ...
            for k in np_vars.keys():
                fdest.create_dataset(k, data=np_vars[k][start_id:end_id])
            fdest.close()
...
@click.command()
@click.option("--root_dir", type=click.Path(exists=True),default='D:/WeatheBench')
@click.option("--save_dir", type=str,default='D:/WeatheBench/1')
@click.option(
    "--variables",
    "-v",
    type=click.STRING,
...
@click.option("--start_train_year", type=int, default=2012)
@click.option("--start_val_year", type=int, default=2017)
@click.option("--start_test_year", type=int, default=2018)
...
def main(
    ...
    # nc2np(root_dir, variables, train_years, save_dir, "train", resolution, num_shards)
    # nc2np(root_dir, variables, val_years, save_dir, "val", resolution, num_shards)
    nc2np(root_dir, variables, test_years, save_dir, "test", resolution, num_shards)

    # save constant values
    constants = xr.open_mfdataset(os.path.join(root_dir, f"constants/constants_{resolution}deg.nc"), combine="by_coords",
                                  parallel=True)
    constant_fields = ["land_sea_mask", "orography", "latitude", "longitude"]
    constant_values = {}
    normalize_constant_mean = {}
    normalize_constant_std = {}
    for f in constant_fields:
        constant_values[f] = np.expand_dims(constants[NAME_TO_VAR[f]].to_numpy(), axis=(0, 1))
        if f == "land_sea_mask":
            normalize_constant_mean[f] = 0 * constant_values[f].mean(axis=(0, 2, 3))
            normalize_constant_std[f] = 1 + 0 * constant_values[f].std(axis=(0, 2, 3))
        else:
            normalize_constant_mean[f] = constant_values[f].mean(axis=(0, 2, 3))
            normalize_constant_std[f] = constant_values[f].std(axis=(0, 2, 3))
    # print(constant_values)
    np.savez(os.path.join(save_dir, "constant.npz"), **constant_values)
    np.savez(os.path.join(save_dir, "constant_mean.npz"), **normalize_constant_mean)
    np.savez(os.path.join(save_dir, "constant_std.npz"), **normalize_constant_std)
